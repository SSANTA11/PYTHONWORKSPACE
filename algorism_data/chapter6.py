...
# 초안
# 기존의 작성한 필드 메서드를 응용하자. 예상하건데 아마 size()가 쓰일 듯....(하다보니 느낀건데 왜 앞선 메서드 작성에선 size를 안썼을까...? 의도와 무관하게 size의 원리를성실히 공부해버렸다;;;)
...
# 수정안1 - 변수 최소화
    def append2(self, e):
        if self.isEmpty() is not True:
            node=self.getNode(self.size()-1) # 빅오가 n^2이 된다... 아마 tail이 있다면 줄일 수도--> 그런데 그렇게 한다면 몇몇 메서드들도 수정이 필요함으로 PASS!
            node.link=Node(e)
        else:
            self.head=Node(e)
